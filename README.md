...
- **Optional image generation** — set `IMAGE_GENERATION=true` and the model can generate images (e.g. "draw me a cat wearing a hat") via the OpenAI Responses API's hosted `image_generation` tool — no new key, it bills through your existing `OPENAI_API_KEY`. The model decides when an image is actually warranted (no separate classifier signal, like actions). Images come back as `images: ["data:image/png;base64,..."]` on the answer and persist with the message. Quality/size/model are configurable (`IMAGE_GENERATION_QUALITY`, default `high`; `IMAGE_GENERATION_SIZE`; `IMAGE_GENERATION_MODEL`); only engages for a resolved OpenAI-served model, and a message with generated images is never written to the response cache.
...
- **Daily spend cap** — set `DAILY_BUDGET_USD` and, once the next call's worst-case cost would push today's total (across all users, per UTC day) past the limit, the call is refused before any model runs. Every billable call is counted in a durable spend log — including empty/truncated reasoning calls that aren't saved as messages. `/v1/status` reports only whether a cap is active; the live figures stay off the public endpoint.
...
| `IMAGE_GENERATION` | `false` | Offer the model the hosted `image_generation` tool (no new key — bills through `OPENAI_API_KEY`). The model decides when an image is warranted. Only engages for a resolved OpenAI-served model. |
| `IMAGE_GENERATION_MODEL` | `gpt-image-1` | The image_generation model. Point this at a newer model as OpenAI ships one, with no code change. |
| `IMAGE_GENERATION_QUALITY` | `high` | `low`\|`medium`\|`high`\|`auto` for generated images. Default favors quality; lower it for a cost-sensitive deployment. |
...
| Method | Path | Body | Response |
| --- | --- | --- | --- |
| `POST` | `/v1/ask` | `{"question": str, "mode": "auto"\|"budget"\|"fast"\|"smart", "no_cache": bool, "model": str\|null}` (`mode` defaults to `"auto"`, `no_cache` to `false`) | `{"answer": str, "mode_used": str, "notes": str, "input_tokens": int\|null, "output_tokens": int\|null, "cost_usd": float\|null, "cached": bool, "sources": [{"title": str, "url": str}]\|null, "pending_action": {"action": str, "summary": str, "payload": object}\|null, "images": [str]\|null}` |
...
| `GET` | `/v1/conversations/{id}/messages` | — | `[{"id": int, "conversation_id": int, "role": str, "content": str, "mode_used": str\|null, "notes": str\|null, "input_tokens": int\|null, "output_tokens": int\|null, "cost_usd": float\|null, "cached": bool, "sources": [{"title": str, "url": str}]\|null, "pending_action": {"action": str, "summary": str, "payload": object}\|null, "action_status": "pending"\|"confirmed"\|"declined"\|"failed"\|null, "images": [str]\|null, "created_at": str}, ...]`; `404` if not found |
...
With `IMAGE_GENERATION=true`, the model is offered the OpenAI Responses API's hosted `image_generation` tool. Unlike web search there's no separate classifier signal — same as actions, the model itself decides when an image is actually warranted (an explicit request like "draw me..."/"generate an image of..."), so nothing changes for ordinary questions. Generated images come back as `images: ["data:image/png;base64,..."]` on the answer and persist with the assistant message; the UI renders them inline. If the model generates an image but produces no other text, a short caption ("Here's the image you asked for.") is synthesized so the reply isn't dropped by the empty-answer guard.

`IMAGE_GENERATION_MODEL` (default `gpt-image-1`), `IMAGE_GENERATION_QUALITY` (default `high`), and `IMAGE_GENERATION_SIZE` (default `auto`) configure the tool call. Cost isn't token-based, so it's tracked separately: `IMAGE_GENERATION_COST_USD` (or a built-in per-quality estimate) is added to the answer's `cost_usd` and the spend log per generated image — note this addition is **not** yet reflected in `DAILY_BUDGET_USD`'s pre-dispatch check, which only prices tokens. Same gating and cache behavior as the other tools: only engages for a resolved OpenAI-served model, and a message with generated images is never written to the response cache.

Same gating as web search: only engages when the resolved model is OpenAI-served, and a message with a pending action is never written to the response cache (a replayed cached answer could re-surface a stale or already-resolved proposal).
