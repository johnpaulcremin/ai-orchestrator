...
- **Bulk archive/delete conversations** — the **Select** button in the sidebar turns every conversation row into a checkbox; pick as many as you like, then **Archive selected** or **Delete selected** to act on the whole batch at once instead of one at a time. Best-effort like Export all/bulk import: each conversation is archived/deleted individually via the same single-item endpoints, so one failure never blocks the rest — the status line reports the split (e.g. "Archived 2 of 3 conversations (1 failed)."). Bulk delete asks for one confirmation covering the whole batch, not one per conversation.
