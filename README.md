...
- **Image input / vision** — attach up to 4 images to a question (the 📎 button in the UI, pasting a screenshot directly into the question box from your clipboard, or `images: [...]` on the request as `data:image/{png,jpeg,gif,webp};base64,...` URLs) and the resolved model sees them alongside the text — no opt-in flag, no new key, works across **every** provider (OpenAI, Anthropic/Claude, and any LiteLLM-routed model whose provider supports vision). Paste goes through the exact same validation/preview path as the 📎 picker (same 4-image cap, same skip-status messaging), and never interferes with a normal text paste — a copied screenshot has no text representation, so nothing is lost either way. Attached images persist with the user's message and are reused automatically on regenerate. A request with attached images is never served from or written to the response cache.
