...
103 features across routing, cost control, attachments, and conversation management — the full list with how/why each one works is in **[docs/features.md](docs/features.md)**. Headline names, grouped:
...
**Auth & UX:** Optional auth + per-user data · Admin user management · Runtime-editable model map · Keyboard shortcuts · Light / dark theme toggle · Background reply notifications · Copy to clipboard · First-run setup wizard · Onboarding hints for a fresh install · SSE streaming · Telemetry · OpenTelemetry tracing · Fixed app-shell layout · Collapsed header + hover-revealed message actions · A shared Button component + design tokens · A hero composer · Baseline security headers
...
- **[docs/remote-access.md](docs/remote-access.md)** — reaching this app from a phone or a second machine over Tailscale, and the required auth step before doing so.
