...
- **Deep-linkable conversations** — the selected conversation is reflected in the URL as `?c=<id>` (via `history.replaceState`, so switching conversations doesn't pile up browser history entries), so reloading the page, bookmarking it, or sharing the link lands back on the same conversation instead of always falling back to the default pick. Loading a `?c=` for a conversation that doesn't exist (deleted, wrong owner, or just made up) falls back to the normal default selection rather than erroring.
