...
- **Background reply notifications** — the 🔕/🔔 button next to the theme toggle opts into being told when an answer finishes while this tab isn't the one you're looking at: the tab title flashes to "💬 New reply" (reverting the moment you switch back), and, if you grant the browser's Notification permission, a native OS notification pops up too, click-to-focus straight into that conversation. Off by default, and only ever fires for the tab that's actually hidden — permission is requested once, right when you turn it on, never on page load unprompted. Entirely client-side (`localStorage`), no new endpoint.
- **Export a conversation** — the ⬇️ Export control in the chat header downloads the current conversation as Markdown (human-readable, with sources linked and attachment counts noted) or JSON (the full raw message data, including attachment content), or opens it as a print-ready **PDF (print)** document in a new tab and hands off to the browser's native print dialog — no PDF library dependency, "Save as PDF" is standard there in every modern browser. All three are entirely client-side — no server round-trip, since the messages are already loaded in the UI.
...
- **Bulk archive/delete/export conversations** — the **Select** button in the sidebar turns every conversation row into a checkbox; pick as many as you like, then **Archive selected**, **Delete selected**, or **Export selected** to act on just that batch instead of one conversation at a time (or all of them, via Export all). Archive/delete are best-effort like Export all/bulk import: each conversation is acted on individually via the same single-item endpoints, so one failure never blocks the rest — the status line reports the split (e.g. "Archived 2 of 3 conversations (1 failed)."). Export selected produces the same Export-all-shaped JSON bundle, just scoped to the checked conversations. Bulk delete asks for one confirmation covering the whole batch, not one per conversation.
- **Usage dashboard** — the **Usage** panel in the chat header (`GET /v1/usage?days=`) reads back your own slice of the same `spend_log` ledger that backs the daily budget cap: today's total, a by-day bar chart, and a by-model breakdown (calls, tokens, cost) over a selectable 7/14/30/90-day window. Owner-scoped — you only ever see your own spend, never the deployment-wide total (that stays off the public `/v1/status`). When `DAILY_BUDGET_PER_OWNER_USD` is set, it also shows how much of *your own* cap is left today (e.g. "$0.65 left of your $1.00 daily cap"); with only `DAILY_BUDGET_USD` set, it shows that configured limit as context. Either way, only the configured limit(s) are exposed here — never the live global spend total, which stays private to the operator.
...
| Method | Path | Body | Response |
| --- | --- | --- | --- |
| `GET` | `/v1/conversations?include_archived=bool` | — | `[{"id": int, "title": str, "pinned_model": str\|null, "system_prompt": str\|null, "favorite": bool, "archived": bool, "tags": [str], "created_at": str, "updated_at": str}, ...]` (favorited conversations first, most recently updated first within each group; archived conversations are excluded unless `include_archived=true`, default `false`) |
...
| `POST` | `/v1/conversations/import` | `{"title": str, "pinned_model": str\|null, "system_prompt": str\|null, "favorite": bool, "tags": [str], "messages": [{"role": "user"\|"assistant", "content": str, "mode_used": str\|null, "notes": str\|null, "input_tokens": int\|null, "output_tokens": int\|null, "cost_usd": float\|null, "cached": bool, "sources": [{"title": str, "url": str}]\|null}, ...]}` (`title` defaults to `"Imported conversation"`; `favorite` defaults to `false`; `tags` defaults to `[]`, same normalization as `PUT .../tags`; 1–500 messages, each content 1–100,000 chars; `pinned_model` validated the same as `PUT .../pin`) | Re-creates a conversation from these messages, in order, with fresh ids and no model calls — restores the pin, instructions, favorite status, tags, and per-message tokens/cost/cached/sources; attachments (images/files) are never restored. The created conversation object; `422` on validation failure |
...
| `GET` | `/v1/conversations/{id}/messages` | — | `[{"id": int, "conversation_id": int, "role": str, "content": str, "mode_used": str\|null, "notes": str\|null, "input_tokens": int\|null, "output_tokens": int\|null, "cost_usd": float\|null, "cached": bool, "sources": [{"title": str, "url": str}]\|null, "pending_action": {"action": str, "summary": str, "payload": object}\|null, "action_status": "pending"\|"confirmed"\|"declined"\|"failed"\|null, "images": [str]\|null, "files": [{"filename": str, "data": str}]\|null, "bookmarked": bool, "created_at": str}, ...]`; `404` if not found |
