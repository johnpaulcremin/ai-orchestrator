...
# own cloud, same trust boundary as web_search) is GA as of code_execution_
# 20250825 and no longer needs an anthropic-beta header at all — the SDK
# still accepts the legacy header as a harmless opt-in, which is why this
# stayed silently "working" (no error) even though it's unnecessary. Beta
# features are still reached via a distinct client namespace (client.
# beta.messages instead of client.messages), so this constant stays as the
# betas=[...] value on that call — see call_anthropic/stream_anthropic
# switching on `code_execution` below.
...
# BUG HISTORY (ground-truthed against the real API, not the SDK's typed
# response classes, which don't surface this): a `code_execution_20250825`
# tool named "code_execution" in the REQUEST does NOT come back as
# "code_execution"-named blocks in the RESPONSE. Anthropic's actual
# response shape for this (current, GA) tool version uses
# "bash_code_execution"/"bash_code_execution_tool_result" for a shell
# command, and "text_editor_code_execution"/"text_editor_code_execution_
# tool_result" for a file create/view/edit — never bare "code_execution".
# The extraction code below previously matched "code_execution_tool_result"/
# "code_execution_result"/"code_execution_output", which is the shape of
# the RETIRED code_execution_20250522 (Python-only) tool — a complete type
# mismatch against every real call using the current tool, which is why
# _extract_anthropic_code_results silently returned [] for every live call
# (not just ones with a generated file): the mocked tests encoded the same
# wrong assumption, so they passed while every real call quietly extracted
# nothing at all. Only bash_code_execution is handled below — a
# text_editor_code_execution call (creating/editing a script's source, not
# running it) has no stdout/logs of its own and isn't a distinct CodeResult
# worth surfacing; a file it happens to produce is only ever actually
# generated by the bash_code_execution run that executes it, which IS
# captured.
#
# The Files API needed to download a generated file is a SEPARATE beta from
# code execution (`files-api-2025-04-14`, not the code-execution beta
# above) — see _download_anthropic_code_file.
_ANTHROPIC_FILES_API_BETA = "files-api-2025-04-14"


def _download_anthropic_code_file(
    client: anthropic.Anthropic, file_id: str
) -> tuple[str, str] | tuple[str, dict[str, object]] | tuple[str, str]:
    """Download one code_execution-generated file via Anthropic's beta Files
    API. Returns `("image", data_url)` for an image, `("file", {"filename",
    "mime_type", "data"})` (see schemas.CodeFile) for a non-image file within
    _CODE_FILE_MIME_ALLOWLIST (a CSV, a saved spreadsheet, ...), or
    `("skipped", reason)` — NEVER a bare None — for an unsupported mime type,
    an oversized file, or a failed download, so a caller always has something
    to surface instead of a silent drop.

    BUG HISTORY: the metadata's own `mime_type` is not always trustworthy —
    Anthropic's code-execution container has been observed reporting a
    generic type (e.g. `application/octet-stream`) for a file it wrote
    itself rather than sniffing its real content, which meant a genuine
    matplotlib-saved PNG could fail BOTH the image check and the allowlist
    check below and get dropped with zero trace. `filename`'s extension
    (also in the metadata response, so no extra round trip) is used as a
    corroborating signal: an image-shaped extension is trusted as an image
    even when `mime_type` doesn't say so, and likewise for a
    _CODE_FILE_MIME_ALLOWLIST-eligible extension. An explicit `mime_type`
    that's ALREADY unambiguous (a real `image/...` or an allowlisted value)
    is still used as-is — the filename guess is only a fallback, never an
    override of a clear signal.

    Two round trips (metadata, then content) because the download response
    itself carries no mime type or filename. Never raises: a failed download
    degrades to one skipped file, not a broken answer — mirrors every other
    extract-and-enrich helper in this module.
    """
    ...
    try:
        metadata = client.beta.files.retrieve_metadata(
            file_id, betas=[_ANTHROPIC_FILES_API_BETA]
        )
        reported_mime = str(getattr(metadata, "mime_type", "") or "")
        filename = str(getattr(metadata, "filename", "") or file_id)

        if (
            reported_mime.startswith("image/")
            or reported_mime in _CODE_FILE_MIME_ALLOWLIST
        ):
            mime_type = reported_mime
        else:
            # The reported mime type is generic/empty/unrecognized — see the
            # BUG HISTORY note above — fall back to what the filename implies.
            mime_type = guess_code_file_mime(filename) or reported_mime
        ...
        return "skipped", f"{file_id} (download failed)"
...
def _extract_anthropic_code_results(
    ...
    code execution is a PAIR: a `server_tool_use` block (name=
    "bash_code_execution", input={"command": ...}) followed later by a
    `bash_code_execution_tool_result` block carrying the same call's
    stdout/stderr plus any generated files (each a `bash_code_execution_
    output` block with only a `file_id` — no inline data, unlike OpenAI's
    code_interpreter) via `tool_use_id`. See the module-level BUG HISTORY
    note above _ANTHROPIC_FILES_API_BETA for why these exact block/type
    names matter — the tool's own request-side name ("code_execution")
    does NOT reappear in the response. A `text_editor_code_execution` call
    (the model authoring/editing a script's source) is deliberately not
    surfaced as its own CodeResult here: it has no stdout/logs of its own,
    and any file it produces is only actually generated by the
    bash_code_execution run that executes it, which IS captured. Matched
    ...
    try:
        ...
        for block in getattr(message, "content", None) or []:
            ...
            if block_type == "server_tool_use":
                if getattr(block, "name", None) != "bash_code_execution":
                    continue
                ...
                file_ids: list[str] = [
                    str(getattr(output, "file_id"))
                    for output in getattr(content, "content", None) or []
                    if getattr(output, "type", None) == "bash_code_execution_output"
                    and getattr(output, "file_id", None)
                ]
                # Visible even when this ends up being zero — the exact
                # "silent drop" this note replaces: a mismatched response
                # shape (Anthropic changes the block/type names again, a
                # new tool version, ...) now shows up in the logs as
                # file_refs=0 on a call that plainly ran code, instead of
                # looking identical to a call that never referenced a file
                # at all.
                logger.info(
                    "code_results.file_refs count=%d tool_use_id=%s",
                    len(file_ids),
                    tool_use_id,
                )
                images: list[str] = []
                ...
                if client is not None:
                    for file_id in file_ids:
                        kind, payload = _download_anthropic_code_file(client, file_id)
                        if kind == "image":
                ...
                results.append(
                    {
                        ...
                        "images": images,
                        "files": files,
                        "file_warnings": warnings or None,
                    }
                )
    except Exception:
