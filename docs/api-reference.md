...
| `GET` | `/v1/retry-cost/summary?days=int` | — | `{"days": int, "overall": stat, "by_category": {category: stat, ...}, "by_tier": {tier: stat, ...}, "by_signal": {signal: {"retries": int, "retry_cost_usd": float}, ...}, "directional_width": float}` where each `stat` is `{"turns": int, "retried_turns": int, "retries": int, "corrections": int, "unpriced_attempts": int, "first_attempt_cost_usd": float, "total_cost_usd": float, "retry_cost_usd": float, "cost_multiplier": float \| null, "retry_rate": float, "retry_rate_ci": [float, float] \| null, "turns_for_directional": int \| null, "reads_as": "no_data"\|"insufficient"\|"directional"}` — this caller's own re-run cost (see Re-run cost visibility in docs/features.md): what the first attempt at each turn cost, what the same turns cost with every retry added back, and how often a turn was retried, all attributed to the turn's ORIGINAL routing decision rather than to whichever attempt answered last. `days` defaults to 14, range 1–90, same convention as `/v1/usage`. Every rate carries its `n` (`turns`), a 95% Wilson interval and a `reads_as` verdict, because on a small deployment the sample is tiny and a bare percentage would read as a finding; `turns_for_directional` is how many turns at the same observed rate would be needed for the interval to fit inside `directional_width`. `by_signal` keeps the retry reasons apart (`regenerated_unrated`, `regenerated_after_downvote`, `regenerated_after_upvote`, `edited`) rather than summing them, since a regeneration with no rating may be taste rather than a quality failure. No rollup union: `retry_log` is never pruned, so detail is the whole history. |
