...
| `POST` | `/v1/conversations/{id}/messages/restore` | Same shape as one `ImportMessage` (`role`, `content`, `mode_used`, `notes`, `input_tokens`, `output_tokens`, `cost_usd`, `cached`, `sources`, `truncated`, `code_results`, `images`, `files`) | Recreates one message (fresh id, no model call) in this conversation — the backing endpoint for Undo after deleting a message. The created `MessageOut`; `404` if the conversation isn't found, `422` on validation failure |
